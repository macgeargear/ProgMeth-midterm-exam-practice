...
public class GameLogic {
    ...
    public boolean isHandPlayable(int playerIndex) {
        // TODO Implement here
    	boolean play = false;
    	for (BaseCard c : GameLogic.getInstance().getPlayerHand(playerIndex)) {
    		if (c.canPlay() == true) {
    			play = true;
    			break;
    		}
    ...
    public ArrayList<BaseCard> draw(int amount) {
    	...
    	ArrayList<BaseCard> drawCards = new ArrayList<>();
    	while (!deck.isEmpty() && amount > 0) {
    		int ind = deck.size()-1;
    		drawCards.add(deck.get(ind));
    		deck.remove(ind);
    		amount--;
    	}
    	return drawCards;
